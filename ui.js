...
/**
 * Определяет тач-устройство по медиазапросу.
 * @returns {boolean}
 */
 ...
 * Используется везде где вставляем пользовательский текст в innerHTML.
 * @param {*} s
 * @returns {string}
 */
 ...
 * @param {boolean} err  — true для стиля ошибки
 */
export function toast(msg, err = false) {
  const tc = document.getElementById("toastContainer");
  if (!tc) {
    /* Fallback если контейнер не найден — не ломаем приложение */
    console[err ? "error" : "log"]("[toast]", msg);
    return;
  }
